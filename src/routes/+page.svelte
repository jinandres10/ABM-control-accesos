<script lang="ts">
	...
	interface NavCard {
		...
		titulo: string;
		descripcion: string;
		href: '/' | '/dashboard' | '/edificios' | '/edificios/nuevo';
		acento: string;
		colorLink: string;
	}
	...
	const navCards: NavCard[] = [
		{
			...
			icono: '🏢',
			titulo: 'Edificios',
			descripcion: 'Gestioná los edificios del sistema: alta, modificación y baja.',
			href: '/edificios',
			acento: 'acento-blue',
			colorLink: '#2563eb'
		}
			...
			stats = [
				{
					...
					sub:
						totalEdificios === 1
							? '1 edificio cargado'
							: `${totalEdificios ?? 0} edificios cargados`
				},
				{
					id: 'perfiles',
					icono: '👤',
					...
					sub:
						totalPerfiles === 1
							? '1 perfil registrado'
							: `${totalPerfiles ?? 0} perfiles registrados`
				},
				{
					id: 'accesos',
					icono: '🔐',
