<script lang="ts">
	...
	function exportarExcel() {
		...
		XLSX.utils.book_append_sheet(workbook, worksheet, 'RegistroPresencia');
	...
	const ingresosFiltrados = $derived(
		[...(data.ingresos ?? [])]
			...
			.filter((i: IngresoEgreso) => {
				...
				const textoBusqueda = filtroUsuario.trim().toLowerCase();
				...
				const cumpleUsuario =
					...
					`${perfil?.nombre ?? ''} ${perfil?.apellido ?? ''}`.toLowerCase().includes(textoBusqueda);
				...
				const cumpleEdificio =
					!filtroEdificio ||
					i.nombre_edificio?.toLowerCase().includes(filtroEdificio.toLowerCase());

				/* =========================
                   FILTRO FECHA
                ========================= */
				const fechaRegistro = normalizarFecha(i.creado_en);

				const cumpleFecha = !filtroFecha || fechaRegistro.includes(filtroFecha);

				return cumpleUsuario && cumpleEdificio && cumpleFecha;
			})
	);
</script>
...
<div class="page">
	...
	<div class="filtros">
		...
		<input type="text" placeholder="🏢 Filtrar por edificio" bind:value={filtroEdificio} />

		<input type="text" placeholder="📅 dd/mm/yyyy" bind:value={filtroFecha} />
	</div>
	...
	<div class="table-wrap">
		<table>
			...
			<tbody>
				...
				{:else}
					<tr>
						<td colspan="8" class="vacio"> No hay registros encontrados </td>
					</tr>
				{/each}
			</tbody>
		</table>
