...
export const POST: RequestHandler = async ({ request }) => {
	...
	if (!email || typeof email !== 'string' || !password) {
		return json({ error: 'Debe ingresar email y contraseña' }, { status: 400 });
	}
	...
	if (loginError) {
		...
		if (nuevosIntentos === MAX_INTENTOS - 1) {
			return json({ error: 'Último intento antes del bloqueo del usuario' }, { status: 401 });
		}
		 ...
		 */
		if (bloquear) {
			return json({ error: 'Usuario bloqueado por múltiples intentos fallidos' }, { status: 403 });
		}
