<script lang="ts">
  ...
  let form: PerfilForm = { nombre: '', email: '' }
  ...
  async function guardar(): Promise<void> {
    ...
    if (editandoId) {
      ...
      const { error: err } = await supabase.from('perfiles').insert(payload)
      if (err) error = err.message
    }
  ...
  function editar(p: Perfil): void {
    editandoId = p.id
    form = { nombre: p.nombre ?? '', email: p.email ?? '' }
  }
      ...
      <tbody>
        {#each data.perfiles as p (p.id)}
          <tr class:editando={editandoId === p.id}>
            ...
            <td class="acciones">
              <button class="btn-icono" on:click={() => editar(p)} title="Editar">✏️</button>
              <button class="btn-icono danger" on:click={() => eliminar(p.id)} title="Eliminar">🗑️</button>
            </td>
          </tr>
        {:else}
          <tr><td colspan="4" class="vacio">No hay perfiles registrados</td></tr>
        {/each}
      </tbody>
    </table>
  </div>
